...
- **`agent-guides/blog/SCRIPT_GUIDE.md`** — For the 2-3 minute Instagram vertical script. Word budget, the six beats, why it is a true script when session notes never are, and what was rejected from the source guide.
- **`agent-guides/blog/SOCIAL_GUIDE.md`** — For LinkedIn and Instagram assets made from published briefings. Cover anatomy and the three design families, headline gradients, the background map, the infographic-fit rule, the UTM convention. Assets are built one at a time on request, never in batch.
- **`.claude/agents/cover.md`**, **`.claude/agents/infographic.md`**, and **`.claude/agents/script.md`** — the three builders. Ask them by name rather than rebuilding the reasoning.
- **`agent-guides/blog/DISTRIBUTION_LOG.md`** — The social manifest and queue. What has been produced, what has shipped, and which briefing is next. Check it before making any social asset; update it after.
...
| Video covers, infographics, or social copy | blog/SOCIAL_GUIDE + blog/DISTRIBUTION_LOG + blog/BLOG_BRIEF + COPY_GUIDE + DESIGN_GUIDE |
| Instagram vertical scripts | blog/SCRIPT_GUIDE + blog/SOCIAL_GUIDE + COPY_GUIDE |
