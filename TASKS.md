...
- [ ] 10. **[Claude] Publish the banked pipeline at 3/week (Mon/Wed/Fri).**
  ...
  **Next slots:** Wed 08-26, suggested `ai-in-the-clinic-what-stays-human` (**zero inbound — add the inbound at publish**; the natural source is `what-to-delegate-to-ai-and-what-to-never`, which already carries the clinical examples in its judgment pile). Reasoning: it closes both open gaps at once. **Industry breadth is now the oldest unfixed item here** — it was flagged on 08-05 as "the gap to fix next week" and the stall means faith, education, government, healthcare, nonprofit, and creators have had nothing since 07-11, seven weeks. Healthcare is the strongest of those to restart on, because `ai-governance` has also not had an outing since 07-31, and the clinic piece is the one draft that is both a fresh industry and the stale theme. It follows today's briefing naturally too: both are about which lines stay human when the patterned work leaves. Fri 08-28 should then take a second non-business industry rather than returning to business. Bank now lasts ~5 weeks at 3/week; refill comes from cohort exhaust (#14) and Search Console (#12), never invented topics.
  **Gate 2 is open on row 16:** the cover headline needs approving as text before any social asset is built (`DISTRIBUTION_LOG.md` rule 0), and rule 8's session-notes question is now live for a real row and still needs Michael's answer.
